...
// Rewritten build: adds Batch Size slider (default 6 gal), Ion Balance Check,
// Kettle Alkalinity Warning, Unit Conversion Helper, and TDS Estimate.
// All water-chemistry helpers below are independent implementations based on
// standard published brewing water chemistry principles (Palmer, Noonan,
// Janish, common homebrew references) — not derived from any third-party
// licensed spreadsheet or software.
// ============================================================================
...
const SOURCE_PRESETS = {
  mywater:  { ca: 50, mg: 13, na: 17, so4: 42, cl: 27, hco3: 173 },
...
// FIX 1 (CRITICAL): Pickling Salt (NaCl) previously declared "na" twice
// (103.9 then 0), which silently zeroed sodium due to duplicate object keys.
// Fixed below.
// FIX 3 (HIGH): gypsum and cacl2 penalty values reduced from 1.0 to 0.15 so
// the optimizer can dose these common, food-safe salts more freely to hit
// target ppm.
// ----------------------------------------------------------------------------
const SALTS = {
  gypsum: { name: "Gypsum", formula: "CaSO4·2H2O", ca: 61.5, so4: 147.4, mg: 0, na: 0, cl: 0, hco3: 0, penalty: 0.15 },
  cacl2:  { name: "Calcium Chloride", formula: "CaCl2·2H2O", ca: 72.0, cl: 127.4, mg: 0, na: 0, so4: 0, hco3: 0, penalty: 0.15 },
  epsom:  { name: "Epsom Salt", formula: "MgSO4·7H2O", mg: 26.1, so4: 103.0, ca: 0, na: 0, cl: 0, hco3: 0, penalty: 0.3 },
  baking: { name: "Baking Soda", formula: "NaHCO3", na: 72.3, hco3: 191.9, ca: 0, mg: 0, so4: 0, cl: 0, penalty: 4.0 },
  mgcl2:  { name: "Magnesium Chloride", formula: "MgCl2·6H2O", mg: 31.6, cl: 92.3, ca: 0, na: 0, so4: 0, hco3: 0, penalty: 2.0 },
  salt:   { name: "Pickling Salt", formula: "NaCl", na: 103.9, cl: 160.3, ca: 0, mg: 0, so4: 0, hco3: 0, penalty: 2.5 },
  lime:   { name: "Slaked Lime", formula: "Ca(OH)2", ca: 143.0, hco3: 435.0, mg: 0, na: 0, so4: 0, cl: 0, penalty: 5.0 }
};
...
// ----------------------------------------------------------------------------
// Ion Conversion Factors (standard published unit conversions)
// ----------------------------------------------------------------------------
const ION_CONVERSION_FACTORS = {
  caFromCaCO3: 0.401,
  mgFromCaCO3: 0.243,
  hco3FromCaCO3: 1.22,
  so4FromSO4S: 3.0
...
// ============================================================================
// APPLICATION STATE
// Default batch size is 6 US gallons, split as ~67% mash / ~33% kettle
// addition water. A batch size slider allows scaling both proportionally.
// ============================================================================
let state = {
  unit: "us",
  recipeName: "Graco's Homebrew Batch",
  sourcePresetKey: "mywater",
  sourceIons: { ca: 50, mg: 13, na: 17, so4: 42, cl: 27, hco3: 173 },
  roRatio: 0,
  targetKey: "janishhazyneipa",
  batchVol: 6.0,      // NEW: total batch size (gal or L), default 6 gallons
  mashVol: 4.0,
  spargeVol: 2.0,     // "Kettle Addition" water — added at boil, not sparge tank
  noSparge: false,
  targetMashPh: 5.25,
  targetSpargePh: 5.60,
  grains: [
...
function setupEventListeners() {
  ...
  document.getElementById("sourcePreset")?.addEventListener("change", (e) => {
    ...
    if (SOURCE_PRESETS[key]) state.sourceIons = { ...SOURCE_PRESETS[key] };
    updateSourceInputs();
    calculateAll();
  });
  ...
  ["ca", "mg", "na", "so4", "cl", "hco3"].forEach(ion => {
    document.getElementById(`src${ion}`)?.addEventListener("input", (e) => {
      state.sourceIons[ion] = parseFloat(e.target.value) || 0;
      if (state.sourcePresetKey === "custom") SOURCE_PRESETS.custom[ion] = state.sourceIons[ion];
      calculateAll();
    });
  });
  ...
  ["ca", "mg", "na", "so4", "cl", "hco3"].forEach(ion => {
    document.getElementById(`tgt${ion}`)?.addEventListener("input", (e) => {
  ...
  document.getElementById("roSlider")?.addEventListener("input", (e) => {
    ...
    const roValEl = document.getElementById("roVal");
    if (roValEl) roValEl.textContent = state.roRatio;
    calculateAll();
  });
  ...
  // NEW: Batch Size Slider — scales mashVol and spargeVol proportionally
  document.getElementById("batchVolSlider")?.addEventListener("input", (e) => {
    const newBatch = parseFloat(e.target.value) || 6.0;
    scaleVolumesToBatch(newBatch);
    syncBatchVolInputs();
    calculateAll();
  });

  document.getElementById("batchVolNumber")?.addEventListener("input", (e) => {
    const newBatch = parseFloat(e.target.value) || 6.0;
    scaleVolumesToBatch(newBatch);
    syncBatchVolInputs();
    calculateAll();
  ...
  document.getElementById("mashVol")?.addEventListener("input", (e) => {
    ...
    state.batchVol = state.mashVol + (state.noSparge ? 0 : state.spargeVol);
    syncBatchVolInputs();
    calculateAll();
  });
  ...
  document.getElementById("noSparge")?.addEventListener("change", (e) => {
    ...
    const grp = document.getElementById("spargeVolGroup");
    if (grp) grp.style.display = state.noSparge ? "none" : "block";
    calculateAll();
  });
  ...
  document.querySelectorAll(".salt-checkbox")?.forEach(cb => {
    cb.addEventListener("change", calculateAll);
  });

  // Unit Converter panel (optional UI)
  ["convCaCO3Ca", "convCaCO3Mg", "convCaCO3Hco3", "convSO4S"].forEach(id => {
    document.getElementById(id)?.addEventListener("input", updateConversionResults);
  });
}
...
function setUnit(u) {
  ...
  const volLbl = document.getElementById("volUnitLbl");
  if (volLbl) volLbl.textContent = u === "us" ? "GAL" : "L";
  const grainLbl = document.getElementById("grainUnitLbl");
  if (grainLbl) grainLbl.textContent = u === "us" ? "LB" : "KG";
  calculateAll();
}

// ============================================================================
// NEW: BATCH SIZE SLIDER LOGIC (default 6 gallons)
// Scales mash and kettle-addition water proportionally when the total batch
// size is changed via slider or numeric input.
// ============================================================================
function scaleVolumesToBatch(newBatchVol) {
  const oldTotal = state.mashVol + (state.noSparge ? 0 : state.spargeVol);
  if (oldTotal <= 0) {
    // Fallback to a standard 67/33 mash/kettle split
    state.mashVol = Math.round(newBatchVol * 0.67 * 10) / 10;
    state.spargeVol = Math.round((newBatchVol - state.mashVol) * 10) / 10;
  } else {
    const ratio = newBatchVol / oldTotal;
    state.mashVol = Math.round(state.mashVol * ratio * 10) / 10;
    state.spargeVol = state.noSparge ? 0 : Math.round(state.spargeVol * ratio * 10) / 10;
  }
  state.batchVol = newBatchVol;
}

function syncBatchVolInputs() {
  const slider = document.getElementById("batchVolSlider");
  const number = document.getElementById("batchVolNumber");
  const mv = document.getElementById("mashVol");
  const sv = document.getElementById("spargeVol");
  if (slider) slider.value = state.batchVol;
  if (number) number.value = state.batchVol;
  if (mv) mv.value = state.mashVol;
  if (sv) sv.value = state.spargeVol;
}

// ============================================================================
// SOURCE / TARGET PRESET UI
// ============================================================================
function populateSourcePresets() {
  const sel = document.getElementById("sourcePreset");
  if (!sel) return;
  sel.innerHTML = `
    <option value="mywater">My Water (Tap Profile)</option>
...
function saveCustomTargetProfile() {
  ...
  const profile = { name: name.trim(), ...currentIons, ph: state.targetMashPh };
...
function loadSavedCustomProfiles() {
  // Profiles are merged into SOURCE_PRESETS / PRESETS lazily by
  // populateSourcePresets() / populatePresets() on load.
}
...
function updateSourceInputs() {
  ...
  ["ca", "mg", "na", "so4", "cl", "hco3"].forEach(ion => {
    ...
    el.style.backgroundColor = isCustom ? "var(--copper-soft)" : "var(--bg)";
    el.style.borderColor = isCustom ? "var(--copper)" : "var(--line-2)";
  });
}
...
function updateTargetInputs() {
  ...
  ["ca", "mg", "na", "so4", "cl", "hco3"].forEach(ion => {
    const el = document.getElementById(`tgt${ion}`);
    if (!el) return;
    el.value = p.ions[ion];
  ...
  if (noteEl) noteEl.innerHTML = `<b>Ref:</b> ${p.book} — Target <b>${p.ph} pH</b> — ${p.note}`;
}
...
function renderGrainBill() {
  ...
  state.grains.forEach((g, idx) => {
    ...
    div.innerHTML = `
      <input type="text" list="maltBookDatalist" value="${escapeXml(g.name)}" placeholder="Malt Name"
        oninput="onMaltNameInput(${idx}, this.value)" onchange="updateGrain(${idx}, 'name', this.value)">
      <input type="number" id="grainweight${idx}" step="0.1" value="${g.weight}"
        onchange="updateGrain(${idx}, 'weight', parseFloat(this.value) || 0)">
      <input type="number" id="graincolor${idx}" step="0.1" value="${g.color}"
        onchange="updateGrain(${idx}, 'color', parseFloat(this.value) || 0)">
      <select id="graintype${idx}" onchange="updateGrain(${idx}, 'type', this.value)">
...
window.onMaltNameInput = function (idx, val) {
  ...
  if (match) {
    ...
    const colorInput = document.getElementById(`graincolor${idx}`);
    const typeSelect = document.getElementById(`graintype${idx}`);
...
function solveSalts(effSource, targetIons, volGal) {
  const activeKeys = Array.from(document.querySelectorAll(".salt-input:checked"))
    .map(cb => cb.value);
  const keysToUse = activeKeys.length > 0 ? activeKeys : state.activeSalts;

  let bestDosages = {};
  keysToUse.forEach(k => bestDosages[k] = 0);
  if (volGal <= 0 || keysToUse.length === 0) return bestDosages;
...
function calcError(dosages, effSource, targetIons, volGal) {
  ...
  // FIX 3 (HIGH): overall penalty multiplier reduced from 5.0 to 1.0.
  // Combined with the lowered gypsum/cacl2 penalty values above, common
  // salts can now dose more freely to hit target ppm, while baking soda,
  // magnesium chloride, pickling salt, and slaked lime remain comparatively
  // discouraged via their higher individual penalty values.
  Object.keys(dosages).forEach(k => {
...
// ----------------------------------------------------------------------------
// AWARENESS ONLY: heuristic mash pH model. A rule-of-thumb estimate based on
// grist color, residual alkalinity, and acid malt %  — not a full ionic
// buffering model. Verify actual mash pH with a calibrated pH meter before
// finalizing a recipe.
// ----------------------------------------------------------------------------
function estimateMashPh(ppm, grains, mashVolGal) {
  ...
  const avgColorL = weightedColor / Math.max(0.001, (totalWeightLb - acidMaltLb));

  let phDi = 5.60;
  if (avgColorL < 3) {
    phDi = 5.70;
  } else if (avgColorL < 15) {
    phDi = 5.70 - (avgColorL - 3) * 0.012;
  } else {
    phDi = 5.45 - (avgColorL - 15) * 0.004;
  }
...
// FIX 4 (MEDIUM): bonus helper — estimate current kettle/sparge water pH
// from source alkalinity instead of assuming a flat 5.90 for every source
// water profile.
...
// ============================================================================
// NEW: WATER CHEMISTRY VALIDATION HELPERS
// Independent implementations based on standard published brewing water
// chemistry principles (ion equivalent weights, alkalinity-as-CaCO3
// conversion factors, TDS estimation). Not derived from any third-party
// licensed spreadsheet or software.
// ============================================================================

// 1. ION BALANCE CHECK
function checkIonBalance(ions) {
  const caMeq = ions.ca / 20.04;
  const mgMeq = ions.mg / 12.15;
  const naMeq = ions.na / 23.0;
  const so4Meq = ions.so4 / 48.03;
  const clMeq = ions.cl / 35.45;
  const hco3Meq = ions.hco3 / 61.02;

  const cationTotal = caMeq + mgMeq + naMeq;
  const anionTotal = so4Meq + clMeq + hco3Meq;
  const diff = cationTotal - anionTotal;

  return {
    cationTotal: Math.round(cationTotal * 100) / 100,
    anionTotal: Math.round(anionTotal * 100) / 100,
    diff: Math.round(diff * 100) / 100,
    balanced: Math.abs(diff) <= 0.5
  };
}

function renderIonBalanceWarning(ions) {
  const result = checkIonBalance(ions);
  const container = document.getElementById("ionBalanceWarning");
  if (!container) return;
  if (!result.balanced) {
    container.style.display = "block";
    container.innerHTML = `⚠️ <b>Water report may be unbalanced</b> — cations ${result.cationTotal} meq/L vs anions ${result.anionTotal} meq/L (diff ${result.diff}). Double check ion units.`;
  } else {
...
// 2. KETTLE / SPARGE ALKALINITY THRESHOLD WARNING
function checkKettleAlkalinity(hco3Ppm) {
  const alkalinityAsCaCO3 = hco3Ppm * 0.82;
  return {
    alkalinity: Math.round(alkalinityAsCaCO3 * 10) / 10,
    exceeds: alkalinityAsCaCO3 > 25
  };
}

function renderKettleAlkalinityWarning(spargeHco3Ppm) {
  const result = checkKettleAlkalinity(spargeHco3Ppm);
  const container = document.getElementById("kettleAlkalinityWarning");
  if (!container) return;
  if (result.exceeds) {
    container.style.display = "block";
    container.innerHTML = `⚠️ <b>Kettle addition water alkalinity ${result.alkalinity} ppm</b> — above 25 ppm. Consider acid addition to reduce tannin extraction risk.`;
  } else {
    container.style.display = "none";
    container.innerHTML = "";
  }
...
function convertIonUnits(value, conversionType) {
  const factor = ION_CONVERSION_FACTORS[conversionType];
  if (!factor) return value;
  return Math.round(value * factor * 100) / 100;
}

function updateConversionResults() {
  const resultsEl = document.getElementById("conversionResults");
  if (!resultsEl) return;
  const caCO3Ca = parseFloat(document.getElementById("convCaCO3Ca")?.value) || 0;
  const caCO3Mg = parseFloat(document.getElementById("convCaCO3Mg")?.value) || 0;
  const caCO3Hco3 = parseFloat(document.getElementById("convCaCO3Hco3")?.value) || 0;
  const so4S = parseFloat(document.getElementById("convSO4S")?.value) || 0;

  const lines = [];
  if (caCO3Ca > 0) lines.push(`Ca: ${convertIonUnits(caCO3Ca, "caFromCaCO3")} ppm`);
  if (caCO3Mg > 0) lines.push(`Mg: ${convertIonUnits(caCO3Mg, "mgFromCaCO3")} ppm`);
  if (caCO3Hco3 > 0) lines.push(`HCO3: ${convertIonUnits(caCO3Hco3, "hco3FromCaCO3")} ppm`);
  if (so4S > 0) lines.push(`SO4: ${convertIonUnits(so4S, "so4FromSO4S")} ppm`);
  resultsEl.innerHTML = lines.join("<br>");
}

// 4. TDS ESTIMATE CROSS-CHECK
function estimateTDS(ions) {
  return Math.round(ions.ca + ions.mg + ions.na + ions.so4 + ions.cl + ions.hco3);
}

function renderTDSEstimate(ions) {
  const el = document.getElementById("tdsEstimate");
  if (!el) return;
  el.textContent = estimateTDS(ions) + " ppm (est.)";
}
...
function calculateAll() {
  ...
  const targetObj = PRESETS[state.targetKey] ? PRESETS[state.targetKey].ions : PRESETS.janishhazyneipa.ions;
  ...
  // FIX 2 (HIGH): merge mash + sparge dosages before computing the
  // combined/displayed profile, so kettle-addition salt additions are
  // reflected in the resulting water report instead of being silently
  // dropped.
  const mergedDosages = {};
  ...
  // FIX 4 (MEDIUM): use state.targetSpargePh instead of a hardcoded value,
  // and estimate the current kettle/sparge pH from source water alkalinity
  ...
  renderIonBalanceWarning(state.sourceIons);
  renderKettleAlkalinityWarning(state.noSparge ? 0 : effSource.hco3);
  renderTDSEstimate(combinedPpm);
...
function renderOutputs(combinedPpm, targetObj, mashPpm, estPh, mashAcid, spargeAcid, mashDosages, spargeDosages) {
  ...
  const ratioEl = document.getElementById("so4clRatio");
  if (ratioEl) ratioEl.textContent = ratio;

  let ratioDesc;
  const ratioNum = parseFloat(ratio);
  if (ratioNum > 2.5) {
    ratioDesc = "<b>Very Dry / Crisp</b> — High sulfate accentuates hop bitterness, crisp finish (Scott Janish / Ray Daniels Burton profile).";
  } else if (ratioNum > 1.5) {
    ratioDesc = "<b>Slightly Dry / Balanced</b> — Suitable for Pale Ales and German Pilsners.";
  } else if (ratioNum > 0.8) {
    ratioDesc = "<b>Balanced Malt/Hops</b> — Ideal baseline for lagers and amber ales.";
  } else {
    ratioDesc = "<b>Very Soft / Full</b> — High chloride enhances pillowy mouthfeel, juicy hop oils, and haze (Scott Janish NEIPA profile).";
  }
  const ratioDescEl = document.getElementById("ratioDesc");
  if (ratioDescEl) ratioDescEl.innerHTML = ratioDesc;
  ...
  ["ca", "mg", "na", "so4", "cl", "hco3"].forEach(ion => {
    ...
    const resEl = document.getElementById(`res${ion}`);
    if (resEl) resEl.textContent = val;
    const tgtValEl = document.getElementById(`tgtval${ion}`);
    if (tgtValEl) tgtValEl.textContent = tgt;
    const diffEl = document.getElementById(`diff${ion}`);
    if (diffEl) diffEl.textContent = diffText;

    const fillEl = document.getElementById(`fill${ion}`);
    if (fillEl) {
      const pct = Math.min(100, Math.max(5, (val / Math.max(1, tgt)) * 50));
      fillEl.style.width = pct + "%";
      fillEl.className = "fill " + (Math.abs(diff) <= 15 ? "ok" : (Math.abs(diff) <= 40 ? "wn" : "bd"));
    }
  });

  const phValEl = document.getElementById("phVal");
  if (phValEl) phValEl.textContent = estPh.toFixed(2);
  ...
  renderDoseCard("mashDosesCard", mashDosages, mashAcid, "Mash Tun");
  if (!state.noSparge) {
    renderDoseCard("spargeDosesCard", spargeDosages, spargeAcid, "Boil Kettle");
  } else {
...
function renderYeastHealthNotes(ppm) {
  ...
  if (ppm.ca < 50) {
    notes.push("⚠️ <b>Low Calcium (&lt;50 ppm)</b> — Yeast (White/Zainasheff) recommends at least 50 ppm Ca²⁺ for yeast cell wall integrity, mash enzyme protection, and clean flocculation.");
  } else {
    notes.push("✅ <b>Calcium &gt;50 ppm</b> — Adequate Ca²⁺ for yeast flocculation and mash alpha-amylase stability.");
  }
...
function renderDoseCard(elementId, dosages, acidMl, label) {
  ...
  Object.keys(dosages).forEach(k => {
    ...
    if (amt > 0 && SALTS[k]) {
      ...
      const div = document.createElement("div");
      div.className = "dose";
      div.innerHTML = `<div class="dn">${SALTS[k].name} (${SALTS[k].formula})</div><div class="dg">${amt} <span>g</span></div>`;
      container.appendChild(div);
    }
  });
  ...
  if (acidMl > 0) {
    ...
    div.innerHTML = `<div class="dn">${acidName}</div><div class="dg">${acidMl} <span>mL</span></div>`;
    container.appendChild(div);
  }
...
function openBrewSheet() {
  ...
  const volUnit = state.unit === "us" ? "GAL" : "L";

  let html = `
    <div class="sheet-head">
      <img class="sh-logo" src="assets/logo-metal.png" alt="Graco's Brewing Logo">
      <h3 class="sh-title">Graco's Water Lab · Brew Day Sheet</h3>
      <div class="sh-meta">
        Recipe: <b>${escapeXml(state.recipeName)}</b> &nbsp; Target: <b>${targetName}</b> &nbsp;
        Date: ${new Date().toLocaleDateString()}
      </div>
    </div>
    <div class="vessel-h">MASH TUN ADDITIONS (${state.mashVol} ${volUnit})</div>
  `;
    ...
    if (amt > 0 && SALTS[k]) {
      html += `
        <label class="check-item">
          ...
          <div class="txt">${SALTS[k].name} (${SALTS[k].formula}) <small>Stir into mash water prior to dough-in</small></div>
        </label>
      `;
    }
  });
  ...
  if (state.dosages.mashAcidMl > 0) {
    ...
    html += `
      <label class="check-item">
        ...
        <div class="txt">${acidName} <small>Target Mash pH: ${state.targetMashPh}</small></div>
      </label>
    `;
  }

  if (!state.noSparge) {
    html += `<div class="vessel-h" style="margin-top:24px">BOIL KETTLE ADDITIONS (${state.spargeVol} ${volUnit})</div>`;
      ...
      if (amt > 0 && SALTS[k]) {
        html += `
          <label class="check-item">
            <input type="checkbox">
            <div class="amt">${amt} <span>g</span></div>
            <div class="txt">${SALTS[k].name} (${SALTS[k].formula}) <small>Add to kettle once full wort volume is collected, before boil</small></div>
          </label>
        `;
      }
    });
    ...
    if (state.dosages.spargeAcidMl > 0) {
      ...
      html += `
        <label class="check-item">
          ...
          <div class="txt">${acidName} <small>Acidify kettle addition water to pH ${state.targetSpargePh}</small></div>
        </label>
      `;
    }
  }
...
// FIX: Uses standard <BATCH_SIZE> tag (with underscore) per BeerXML 1.0 spec,
// with a fallback check for legacy <BATCHSIZE> exports from earlier builds
// of this app, so both standard and self-exported files import correctly.
// ============================================================================
function saveBeerXml() {
  const batchL = state.unit === "us"
    ? (state.mashVol + state.spargeVol) * 3.78541
    : (state.mashVol + state.spargeVol);
  ...
  xml += `<MASH>\n<NAME>Water Lab Mash Profile</NAME>\n<VERSION>1</VERSION>\n<PH>${state.targetMashPh}</PH>\n</MASH>\n`;
  xml += `</RECIPE>\n</RECIPES>\n`;
...
function loadBeerXml(e) {
  ...
  reader.onload = function (evt) {
    ...
    if (nameNode) state.recipeName = nameNode.textContent.trim();
    ...
    if (fermentables && fermentables.length > 0) {
      ...
      fermentables.forEach(f => {
        ...
        const weight = state.unit === "us"
          ? Math.round(amtKg * 2.20462 * 10) / 10
          : Math.round(amtKg * 10) / 10;

        let gtype = "base";
        if (ftypeStr.includes("acid") || fname.toLowerCase().includes("acid")) gtype = "acid";
        else if (colorSrm > 100) gtype = "roast";
        else if (colorSrm > 15) gtype = "crystal";

        if (weight > 0) state.grains.push({ name: fname, weight: weight, color: colorSrm, type: gtype });
      });
      renderGrainBill();
    }
    ...
    // FIX: check standard <BATCH_SIZE> (BeerXML 1.0 spec) first, falling
    // back to legacy <BATCHSIZE> from earlier exports of this same app.
    const batchSizeNode = recipeNode.querySelector("BATCH_SIZE") || recipeNode.querySelector("BATCHSIZE");
    if (batchSizeNode && batchSizeNode.textContent) {
      ...
      if (batchL > 0) {
        ...
        scaleVolumesToBatch(Math.round(totalGal * 10) / 10);
        syncBatchVolInputs();
      }
    }
